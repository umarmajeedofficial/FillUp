...
if st.button("Process"):
    if audio_files and pdf_file:
        ...
        # Function to save responses to PDF in memory
        def save_responses_to_pdf_in_memory(responses):
            buffer = BytesIO()
        ...
        # Process audio files and generate responses
        responses = []
        ...
        output_text.text_area("Output", "\n\n".join(responses), height=400)

        # Prepare the PDF for download
        pdf_buffer = save_responses_to_pdf_in_memory(responses)
        st.download_button(
            label="Download PDF",
            data=pdf_buffer,
